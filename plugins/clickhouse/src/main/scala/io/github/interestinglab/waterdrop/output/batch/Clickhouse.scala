...
import io.github.interestinglab.waterdrop.config.{Config, ConfigFactory, ConfigRuntimeException, TypesafeConfigUtils}
...
class Clickhouse extends BaseOutput {
  ...
  override def preProcess(df: Dataset[Row]): Unit = {
  ...
  override def afterProcess(df: Dataset[Row]): Unit = {
  ...
  private def renderBaseTypeStatement(
    ...
    val v = String.valueOf(item.get(fieldIndex))
    fieldType match {
      case "DateTime" | "Date" | "String" =>
        statement.setString(index + 1, v)
      case "Int8" | "UInt8" | "Int16" | "UInt16" | "Int32" =>
        statement.setInt(index + 1, v.toInt)
      case "UInt32" | "UInt64" | "Int64" =>
        statement.setLong(index + 1, v.toLong)
      case "Float32" => statement.setFloat(index + 1, v.toFloat)
      case "Float64" => statement.setDouble(index + 1, v.toDouble)
      case Clickhouse.arrayPattern(_) =>
        statement.setArray(index + 1, item.getAs[WrappedArray[AnyRef]](fieldIndex))
      case "Decimal" => statement.setBigDecimal(index + 1, new BigDecimal(v))
      case _ => statement.setString(index + 1, v)
    }
  }
