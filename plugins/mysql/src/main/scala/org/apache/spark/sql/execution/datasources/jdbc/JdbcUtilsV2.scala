 ...
 */
object JdbcUtilsV2 extends Logging {

  def createJdbcOptionsInWrite(cfg: Config): JdbcOptionsInWrite = {
    ...
    println("[info] jdbc url is " + url)


    val extraOptions = new scala.collection.mutable.HashMap[String, String]

    if(cfg.hasPath(JDBCOptions.JDBC_DRIVER_CLASS)){
      extraOptions += (JDBCOptions.JDBC_DRIVER_CLASS -> cfg.getString(JDBCOptions.JDBC_DRIVER_CLASS))
    }else{
      extraOptions += (JDBCOptions.JDBC_DRIVER_CLASS -> "com.mysql.cj.jdbc.Driver")
    }

    extraOptions += (JDBCOptions.JDBC_URL -> url, "dbtable" -> cfg.getString("table"))
    extraOptions += ("user" -> cfg.getString("user"))
    extraOptions += ("password" -> cfg.getString("password"))
    extraOptions += ("save_mode" -> cfg.getString("save_mode"))
    if(cfg.hasPath("update_fields")){
      extraOptions += ("update_fields" -> cfg.getString("update_fields"))
    }

    if(cfg.hasPath(JDBCOptions.JDBC_BATCH_INSERT_SIZE)){
      extraOptions += (JDBCOptions.JDBC_BATCH_INSERT_SIZE -> cfg.getString(JDBCOptions.JDBC_BATCH_INSERT_SIZE))
    }
    if(cfg.hasPath(JDBCOptions.JDBC_NUM_PARTITIONS)){
      extraOptions += (JDBCOptions.JDBC_NUM_PARTITIONS -> cfg.getString(JDBCOptions.JDBC_NUM_PARTITIONS))
    }
    if(cfg.hasPath(JDBCOptions.JDBC_TXN_ISOLATION_LEVEL)){
      extraOptions += (JDBCOptions.JDBC_TXN_ISOLATION_LEVEL -> cfg.getString(JDBCOptions.JDBC_TXN_ISOLATION_LEVEL))
    }else{
      extraOptions += (JDBCOptions.JDBC_TXN_ISOLATION_LEVEL -> "NONE")
    }
    new JdbcOptionsInWrite(CaseInsensitiveMap(extraOptions.toMap))
  }
  ...
  def getOnDuplicateUpdateStatement(updateColumns: Option[String]): String = {
    if (updateColumns.nonEmpty) {
      val updateFields = updateColumns.get.split(",")
      updateFields.map(field => s"$field = IF(VALUES($field) IS NULL, $field,  VALUES($field))").mkString(",")
    } else {
      ""
    }
  }
  ...
  def saveTable(
    ...
    var insertStmt = getInsertStatement(table, rddSchema, tableSchema, isCaseSensitive, dialect)

    if ("overwrite".equals(options.parameters.getOrElse("save_mode", "append"))) {
      println("save mode is " + options.parameters.getOrElse("save_mode", "append") )
      val updateFields = options.parameters.get("update_fields")
      insertStmt += " on duplicate key update " + getOnDuplicateUpdateStatement(updateFields)
    }
    ...
    println("[info] final inserted dataframe partition is : " + repartitionedDF.rdd.partitions.size)
